...
export async function fetchGameDetail(
  ...
  // API 응답: { game: { id, title, type, thumbnail_image_url, items, created_by } }
  ...
  const normalizedGame: Game & { items: GameItem[] } = {
    ...
    items: resolvedItems,
    created_by: game.created_by ?? undefined,
  };
...
export type AdminResultSummaryEntry = {
  id: number | null;
  name: string;
  count: number;
};
...
export type AdminResultSummary = {
  ...
  ranking: AdminResultSummaryEntry[];
};
...
export async function fetchAdminResultSummary(gameId: number): Promise<AdminResultSummary> {
  const response = await requestWithMeta(
    apiClient.get<ApiResponse<AdminResultSummary>>("/games/admin/results/", {

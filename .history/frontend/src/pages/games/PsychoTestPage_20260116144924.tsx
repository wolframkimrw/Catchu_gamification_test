...
import { fetchGamesList, fetchGameDetail, type GameDetailData } from "../../api/games";
...
export function PsychoTestPage() {
  ...
  const [gameData, setGameData] = useState<GameDetailData["game"] | null>(null);
    ...
    fetchGamesList()
      .then((games) => {
        ...
        if (match) {
          setGameId(match.id);
          // 게임 상세 정보 조회
          return fetchGameDetail(match.id);
        }
      })
      .then((gameDetail) => {
        if (gameDetail) {
  ...
  return (
    <>
      {!started ? (
        <GameStartScreen
          game={gameData || { title: template?.title || "심리테스트", type: "PSYCHO_TEST" }}
          onStart={handleStart}
        />
      ) : (

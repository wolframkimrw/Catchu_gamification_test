    ...
    >
      ...
      {showJsonSection ? (
        isSajuJson ? (
          ...
          <GameJsonEditor
            jsonPath={jsonPath!}
            gameSlug={game?.slug || "major-arcana"}
            gameType={game?.type || "PSYCHO_TEST"}
            onCancel={() => setJsonRaw("")}
          />
        ) : null}
        {/* OLD_PSYCHO_EDITOR_START
        ) : isPsychoJson ? (
        ...
        <>
          ...
          <section className="admin-games-list">
            ...
            {game.items.map((item) => (
              <div
                key={item.id}
                className={`admin-games-card ${selectedItemIds.includes(item.id) ? "admin-card-selected admin-card-selected-delete" : ""
                  }`}
                onClick={
                  isDeleteMode
                    ? () =>
